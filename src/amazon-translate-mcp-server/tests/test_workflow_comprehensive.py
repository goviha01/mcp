...
class TestSmartTranslationWorkflowComprehensive:
    ...
    async def test_smart_translate_workflow_translation_error(self, orchestrator):
        ...
        orchestrator.translation_service.translate_text.side_effect = TranslationError("Translation service error")
    ...
    async def test_smart_translate_workflow_validation_error(self, orchestrator):
        ...
        orchestrator.translation_service.validate_translation.side_effect = ValidationError("Validation service error")
...
class TestBatchTranslationWorkflowComprehensive:
    ...
    async def test_managed_batch_translation_workflow_success(self, orchestrator):
        ...
        job_statuses = [
            TranslationJobStatus(job_id="job-123", job_name="test-job", status="SUBMITTED", progress=0.0, created_at=datetime.now()),
            TranslationJobStatus(job_id="job-123", job_name="test-job", status="IN_PROGRESS", progress=50.0, created_at=datetime.now()),
    ...
    async def test_managed_batch_translation_workflow_with_terminology(self, orchestrator):
        ...
        orchestrator.batch_manager.get_translation_job.return_value = TranslationJobStatus(
            job_id="job-456", job_name="test-job", status="COMPLETED", progress=100.0, created_at=datetime.now(), completed_at=datetime.now()
        )
        
        # Mock metrics
        mock_metrics = LanguageMetrics(language_pair="en-es", time_range="24h", translation_count=50, character_count=2500, average_response_time=120.0)
        orchestrator.language_operations.get_language_metrics.return_value = mock_metrics
    ...
    async def test_managed_batch_translation_workflow_failed_job(self, orchestrator):
        ...
        language_pairs = [LanguagePair(source_language="en", target_language="es", supported_formats=["text/plain"])]
        orchestrator.language_operations.list_language_pairs.return_value = language_pairs
        orchestrator.terminology_manager.list_terminologies.return_value = {'terminologies': [], 'next_token': None}
        
        # Mock batch job that fails
        orchestrator.batch_manager.start_batch_translation.return_value = "failed-job-789"
        orchestrator.batch_manager.get_translation_job.return_value = TranslationJobStatus(
            job_id="failed-job-789", job_name="failed-job", status="FAILED", progress=25.0, created_at=datetime.now()
        )
    ...
    @pytest.mark.asyncio
    async def test_managed_batch_translation_workflow_unsupported_language_pair(self, orchestrator):
        """Test workflow with unsupported language pair."""
        # Mock empty language pairs (unsupported)
        orchestrator.language_operations.list_language_pairs.return_value = []
