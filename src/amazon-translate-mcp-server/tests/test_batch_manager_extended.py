...
class TestBatchJobManagerExtended:
    ...
    def test_start_batch_translation_throttling_error(self, batch_manager, mock_aws_client_manager):
    ...
    def test_start_batch_translation_service_quota_exceeded(self, batch_manager, mock_aws_client_manager):
    ...
    def test_stop_translation_job_success(self, batch_manager, mock_aws_client_manager):
        ...
        mock_translate_client.stop_text_translation_job.assert_called_once_with(JobId="job-to-stop")
...
class TestS3ValidationExtended:
    ...
    def test_validate_s3_access_output_bucket_creation_failed(self, batch_manager):
        ...
        mock_s3_client.head_bucket.side_effect = [
            {},  # Input bucket exists
            ClientError({'Error': {'Code': 'NoSuchBucket'}}, 'HeadBucket')  # Output bucket doesn't exist
        ]
        mock_s3_client.list_objects_v2.return_value = {'Contents': [{'Key': 'test.txt'}]}
    ...
    def test_validate_s3_access_output_bucket_creation_access_denied(self, batch_manager):
        ...
        mock_s3_client.head_bucket.side_effect = [
            {},  # Input bucket exists
            ClientError({'Error': {'Code': 'AccessDenied'}}, 'HeadBucket')  # Output bucket access denied
        ]
        mock_s3_client.list_objects_v2.return_value = {'Contents': [{'Key': 'test.txt'}]}
