...
class TestServerToolsExecution:
    ...
    def setup_services(self):
        ...
        with patch.object(server, 'translation_service') as mock_trans, \
    ...
    async def test_translate_text_tool_service_not_initialized(self):
        ...
        original_service = server.secure_translation_service
        ...
        try:
            ...
            server.secure_translation_service = original_service
    
    @pytest.mark.asyncio
    async def test_translate_text_tool_exception_handling(self):
        """Test translate_text exception handling."""
        self.mock_services['secure'].translate_text.side_effect = TranslationError("Translation failed")
...
class TestSeparateBatchTools:
    ...
    async def test_monitor_batch_translation_tool_success(self):
        ...
        job_statuses = [
            TranslationJobStatus(job_id="job-456", job_name="test-job", status="IN_PROGRESS", progress=25.0, created_at=datetime.now()),
            TranslationJobStatus(job_id="job-456", job_name="test-job", status="IN_PROGRESS", progress=50.0, created_at=datetime.now()),
            TranslationJobStatus(job_id="job-456", job_name="test-job", status="COMPLETED", progress=100.0, created_at=datetime.now(), completed_at=datetime.now())
        ]
