...
class TestServerMissingLines:
    ...
    async def test_import_terminology_success_path(self):
        ...
        with patch('awslabs.amazon_translate_mcp_server.server.terminology_manager') as mock_term_mgr:
            with patch('tempfile.NamedTemporaryFile') as mock_temp_file:
                with patch('os.path.exists') as mock_exists:
                    with patch('os.unlink') as mock_unlink:
                        with patch('asyncio.get_event_loop') as mock_loop:
                            # Setup mocks
                            mock_temp_file.return_value.__enter__.return_value.name = '/tmp/test.csv'
                            mock_exists.return_value = True
                            mock_term_mgr.import_terminology.return_value = 'arn:test'
                            
                            mock_executor = MagicMock()
                            mock_executor.return_value = 'arn:test'
                            mock_loop.return_value.run_in_executor = AsyncMock(return_value='arn:test')
    ...
    async def test_get_language_metrics_missing_lines(self):
        ...
        with patch('awslabs.amazon_translate_mcp_server.server.language_operations', None):
            ...
            result = await get_language_metrics(
                ctx=mock_ctx,
                language_pair='en-es'
            )
...
class TestBatchManagerMissingLines:
    ...
    def test_batch_manager_error_handling(self):
        ...
        from awslabs.amazon_translate_mcp_server.models import JobConfig, BatchInputConfig, BatchOutputConfig
...
class TestRetryHandlerMissingLines:
    ...
    def test_async_retry_decorator(self):
        ...
        from awslabs.amazon_translate_mcp_server.retry_handler import with_async_retry, RetryConfig
