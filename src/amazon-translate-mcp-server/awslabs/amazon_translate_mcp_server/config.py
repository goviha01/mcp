...
def load_config_from_env() -> ServerConfig:
    ...
    return ServerConfig(
        ...
        allowed_file_extensions=parse_set(
            os.getenv('TRANSLATE_ALLOWED_EXTENSIONS'),
            {'.csv', '.tmx', '.txt'}
        ),
...
def setup_logging(config: ServerConfig):
    ...
    logger.info(f"Security features - PII: {config.enable_pii_detection}, "
...
def validate_aws_config(config: ServerConfig) -> bool:
    ...
    if not (has_explicit_creds or has_profile or has_env_creds):
        logging.warning(
            ...
            "Relying on IAM roles or instance metadata."
        )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"AWS Translate connectivity verified. "
        ...
        logger.info(f"Security configuration: PII detection={security_config.enable_pii_detection}, "
