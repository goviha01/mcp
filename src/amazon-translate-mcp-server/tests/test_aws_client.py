...
class TestAWSClientManager:
    ...
    def test_init_with_explicit_parameters(self, mock_session, mock_sts_client, clean_env):
        ...
        assert manager._aws_access_key_id == 'test-key'
        assert manager._aws_secret_access_key == 'test-secret'
        assert manager._aws_session_token == 'test-token'
    ...
    def test_init_with_environment_variables(self, mock_session, mock_sts_client, clean_env):
        ...
        os.environ['AWS_SESSION_TOKEN'] = 'env-token'
        ...
        assert manager._aws_access_key_id == 'env-key'
        assert manager._aws_secret_access_key == 'env-secret'
        assert manager._aws_session_token == 'env-token'
    ...
    def test_init_partial_credentials_error(self, clean_env):
        ...
        with patch('awslabs.amazon_translate_mcp_server.aws_client.boto3.Session') as mock_session:
            mock_session.side_effect = PartialCredentialsError(provider='env', cred_var='AWS_SECRET_ACCESS_KEY')
    ...
    def test_get_translate_client_success(self, mock_session, mock_sts_client, clean_env):
        ...
        mock_translate_client.list_languages.assert_called_once_with(DisplayLanguageCode='en', MaxResults=1)
